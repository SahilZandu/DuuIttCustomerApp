...
const MapRouteTracking = ({ mapContainerView, origin, destination, isPendingReq, riderCustomerDetails,onKmsTime }) => {
  ...
  const validateCoordinate = useCallback((lat, lng) => {
...
useEffect(() => {
  ...
  useEffect(() => {
    if (!origin || !destination || !isMountedRef.current || isInitializedRef.current) return;
    ...
    if (isMountedRef.current) {
      ...
      setDestinationCoordinate({ latitude: destLat, longitude: destLng });
