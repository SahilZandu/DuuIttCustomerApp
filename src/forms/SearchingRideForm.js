...
//   const {paymentMethod,totalAmount,screenName} = route.params;
...
//   console.log('paymentMethod--', paymentMethod, addParcelInfo,searchArrive);
...
const SearchingRideForm = ({ navigation, route, screenName }) => {
  ...
  useEffect(() => {
    ...
    if (runingBike && isMountedRef.current) {
      setRideProgessImage(hp('1%'));
      setRideProgess(0.2);
      rideProgessRide = 0.2
      const intervalId = setInterval(() => {
